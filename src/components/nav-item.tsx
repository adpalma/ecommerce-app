...
export const NavItem = ({onClick, label, NavIcon, SelectedNavIcon, isSelected}: NavItemProps) => {
    const SelectedIcon = SelectedNavIcon ? SelectedNavIcon : NavIcon
    return <li className="nav-item" onClick={onClick}>
        {isSelected ? <SelectedIcon style={{ height: "24px", width: "24px" }}/> : <NavIcon style={{ height: "24px", width: "24px" }}/>}
        <div>{label}</div>
    </li>
}